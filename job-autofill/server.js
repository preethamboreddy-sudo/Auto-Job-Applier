...
app.post('/api/profile/:userId', (req, res) => {
    ...
    const params = [
        profileData.firstName || null, profileData.middleName || null, profileData.lastName || null,
        profileData.gender || null, profileData.phone || null, profileData.dob || null,
        profileData.address || null, profileData.preferredLocation || null, profileData.nationality || null,
        profileData.aadharId || null, profileData.college || null, profileData.degree || null,
        profileData.stream || null, profileData.cgpa || null, profileData.percentage12 || null,
        profileData.percentage10 || null, profileData.experience || null, profileData.skills || null,
        hasResume, hasAadhar, userId
    ];
...
app.post('/api/jobs', (req, res) => {
    const { title, company, location, type, salary, logo, description, skills } = req.body;
    const id = 'job-' + Date.now();
    const skillsStr = Array.isArray(skills) ? skills.join(', ') : skills;
    
    const query = `INSERT INTO jobs (id, title, company, location, type, salary, logo, description, skills) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`;
    const params = [id, title, company, location, type, salary, logo, description, skillsStr];
...
app.post('/api/applications', (req, res) => {
    ...
    db.run(`INSERT INTO applications (user_id, job_id) VALUES (?, ?)`, [userId, jobId], function(err) {
        if (err) return res.status(500).json({ error: err.message });
