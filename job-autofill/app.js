...
const app = {
    ...
    saveLocalSession() {
        if (this.state.currentUser) {
            localStorage.setItem('jobapp_user', JSON.stringify(this.state.currentUser));
        } else {
    ...
    async fetchProfile() {
        ...
        try {
            const res = await fetch(`${this.API_URL}/profile/${this.state.currentUser.id}`);
            if (res.ok) {
    ...
    wrapMainContainer(shouldWrap) {
        ...
        sections.forEach(sec => {
            if(shouldWrap && !sec.classList.contains('auth-view-content') && sec.parentElement.id !== 'landing-view') {
        ...
        if (brand) {
            brand.onclick = () => this.navigate(this.state.currentUser ? 'jobs-view' : 'landing-view');
        }
    },
        ...
        if (authForm) {
            authForm.addEventListener('submit', async (e) => {
                ...
                try {
                    ...
                    if (Object.keys(this.state.profile).length > 2 || Object.keys(this.state.profile).filter(k => this.state.profile[k] !== null).length > 2) {
                        this.navigate('jobs-view');
                    } else {
                        this.navigate('profile-view');
                    }
                    ...
                    submitBtn.innerText = titleText.includes("Login") ? "Login" : "Sign Up";
                }
            });
    ...
    renderJobs() {
        ...
        if (this.mockJobs.length === 0) {
            container.innerHTML = '<p class="text-center">No jobs available at the moment.</p>';
            return;
        }
        ...
        this.mockJobs.forEach(job => {
            ...
            const extraSkills = skillsArray.length > 3 ? `+${skillsArray.length - 3}` : '';

            let skillsHtml = displaySkills.map(s => `<span class="skill-tag">${s}</span>`).join('');
            if (extraSkills) skillsHtml += `<span class="skill-tag">${extraSkills}</span>`;
    ...
    viewJobDetails(jobId) {
        ...
        if (container) {
            const skillsArray = Array.isArray(job.skills) ? job.skills : job.skills.split(',').map(s => s.trim());
            const skillsHtml = skillsArray.map(s => `<span class="skill-tag">${s}</span>`).join('');
    ...
    async processApplication(type) {
        ...
        const filledFieldsCount = Object.keys(this.state.profile).filter(k => this.state.profile[k] !== null && this.state.profile[k] !== '').length;
        if (filledFieldsCount < 3) {
            alert("Your profile is incomplete. Please complete your profile to use auto-fill.");
            this.navigate('profile-view');
            return;
        }
        ...
        try {
            const res = await fetch(`${this.API_URL}/applications`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ userId: this.state.currentUser.id, jobId: this.state.selectedJobId })
            });
    ...
    async renderApplicationHistory() {
        ...
        try {
            const res = await fetch(`${this.API_URL}/applications/user/${this.state.currentUser.id}`);
            ...
            apps.forEach(app => {
                const date = new Date(app.applied_at).toLocaleDateString('en-US', { year: 'numeric', month: 'short', day: 'numeric' });
                const statusColor = app.status === 'Accepted' ? 'var(--success-color)' : (app.status === 'Rejected' ? '#ff7b72' : 'var(--primary-color)');
    ...
    async renderAdminDashboard() {
        ...
        container.innerHTML = '<p class="text-center">Loading all applications...</p>';
        ...
        try {
            ...
            apps.forEach(app => {
                ...
                const name = app.firstName ? `${app.firstName} ${app.lastName}` : app.email;
    ...
    async updateApplicationStatus(appId, newStatus) {
        try {
            const res = await fetch(`${this.API_URL}/admin/applications/${appId}/status`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
